...
public class BoltEffect {
    ...
    public interface SegmentSpreader {
         ...
         */
        SegmentSpreader NO_MEMORY = (perpendicularDist, randVec, maxDiff, scale, progress, rand) -> randVec.multiply(maxDiff * rand);
        ...
        static SegmentSpreader memory(float memoryFactor) {
            return (perpendicularDist, randVec, maxDiff, spreadScale, progress, rand) -> {
                ...
                if (length > maxDiff) {
                    perpendicularDist = perpendicularDist.multiply(maxDiff / length);
                }
