...
public class GuiRadialSelector<TYPE extends Enum<TYPE> & IRadialSelectorEnum<TYPE>> extends Screen {
    ...
    public void render(@Nonnull MatrixStack matrix, int mouseX, int mouseY, float partialTick) {
        // center of screen
        float centerX = minecraft.getWindow().getScaledWidth() / 2F;
        float centerY = minecraft.getWindow().getScaledHeight() / 2F;
        ...
        for (TYPE type : types) {
            ...
            // draw label
            matrix.push();
            int width = minecraft.textRenderer.getWidth(type.getShortText());
            matrix.translate(x, y, 0);
            matrix.scale(0.6F, 0.6F, 0.6F);
            minecraft.textRenderer.draw(matrix, type.getShortText(), -width / 2F, 8, 0xCCFFFFFF);
            matrix.pop();
            position++;
        }
