...
public class Coord4D {//TODO - V11: Continue working on replacing uses of this with BlockPos/GlobalPos where appropriate
    ...
    public Coord4D(Entity entity) {
        BlockPos entityPosition = entity.getBlockPos();
    ...
    public Coord4D(BlockHitResult mop, World world) {
     ...
     */
    public static Coord4D get(BlockEntity tile) {
    ...
    public static Coord4D read(CompoundTag tag) {
        return new Coord4D(tag.getInt(NBTConstants.X), tag.getInt(NBTConstants.Y), tag.getInt(NBTConstants.Z),
              RegistryKey.of(Registry.DIMENSION, new Identifier(tag.getString(NBTConstants.DIMENSION))));
    }
    ...
    public static Coord4D read(PacketByteBuf dataStream) {
        return new Coord4D(dataStream.readBlockPos(), RegistryKey.of(Registry.DIMENSION, dataStream.readIdentifier()));
    }
